...
def load_train(image_size=33, label_size=21, stride=14, scale=3):
...
def load_test(scale=3):
    ...
    for img in pre_tests:
        ...
        ret[:, :, 2] = img
        img = ret.copy()
        #img = np.resize(img, (h1, w1, 3))
        # img = np.concatenate((img, img, img),axis=2)
        #img = cv2.cvtColor(img,img, img, cv2.COLOR_GRAY2BGR)
        tests[i]=img
        i += 1
    
    pre_tests = [img.reshape(img.shape[0],img.shape[1],1) for img in pre_tests]
    tests = [img.reshape(img.shape[0],img.shape[1], img.shape[2],1) for img in tests]
