...
def load_test(scale=3, test_folder='test', color_dim=1):
    dirname = './TestDatasets/'+test_folder
    dir_list = os.listdir(dirname)
    ...
    for img in tests:
        ...
        if color_dim>1:
            ret = np.empty((h0, w0, 3), dtype=np.uint8)
            ret[:, :, 0] = img
            ret[:, :, 1] = img
            ret[:, :, 2] = img
            img = ret.copy()
    ...
    pre_tests = [img.reshape(img.shape[0],img.shape[1],color_dim) for img in pre_tests]
    tests = [img.reshape(img.shape[0],img.shape[1],color_dim) for img in tests]
